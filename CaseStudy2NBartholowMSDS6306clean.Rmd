...
table(NB_Test,DFValRel$Attrition)
...
a <- rfe(x=DFTrainFS, y=TrainAttrition, sizes = c(2,5), maxit = 10, metric = "Accuracy" , rfeControl = ctrl, testx = DFValFS)
...
table(NB_Prediction,training_data$Attrition)

test_data <- DFTest

NB_Test <- predict(Naive_Bayes_Model, test_data)

table(NB_Test,test_data$Attrition)
...
table(New_NB_Prediction,training_data$Attrition)
table(New_NB_Prediction2,training_data$Attrition)

test_data <- DFTest[,-1] #Eliminate ID
Test_Data_NB <- naiveBayes(Attrition ~ OverTime + YearsInCurrentRole + StockOptionLevel + YearsAtCompany + TotalWorkingYears, data = test_data)

Test_Data_NB
Test_Data_NB_Pred <- predict(Test_Data_NB, test_data[,c('OverTime','YearsInCurrentRole','StockOptionLevel','YearsAtCompany', 'TotalWorkingYears')])



Test_Data_NB2 <- naiveBayes(Attrition ~ OverTime + TotalWorkingYears + YearsAtCompany + StockOptionLevel + MaritalStatus, data = test_data)

Test_Data_NB2

Test_Data_NB_Pred2 <- predict(Test_Data_NB2, test_data[,c('OverTime','TotalWorkingYears','YearsAtCompany','StockOptionLevel','MaritalStatus')])
...
confusionMatrix(table(Test_Data_NB_Pred2,test_data$Attrition))
